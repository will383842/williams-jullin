// src/pages/MyStory.tsx
import React, { useState } from 'react';
import { useTranslation } from 'react-i18next';
import SEO from '../seo/SEO'; // si ton SEO est un named export, remplace par: import { SEO } from '../seo/SEO';
import { buildAlternates, canonical } from '../seo/helpers';
import { type Locale } from '../router/paths';
import { metaStory } from '../seo/meta';
...
const MyStory: React.FC<MyStoryProps> = ({ navigate }) => {
  // === i18n & SEO (une seule init) ===
  const { t, i18n } = useTranslation();
  const locale = (i18n?.language?.split('-')[0] ?? 'fr') as Locale;
  const bcp47 = locale === 'fr' ? 'fr-FR' : `${locale}-${locale.toUpperCase()}`;
  const alternates = buildAlternates(locale, 'story');
  const can = canonical(locale, 'story');
  const meta = metaStory(locale);
                      ...
                      <div className="flex items-center space-x-3 lg:space-x-4 mb-4 lg:mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-red-100 text-sm lg:text-base">
                            {t('story.platforms.sos.subtitle')}
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center space-x-3 lg:space-x-4 mb-4 lg:mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-blue-100 text-sm lg:text-base">
                            {t('story.platforms.ulixai.subtitle')}
                          </p>
                        </div>
                      </div>
                    </div>
